...
let
  ...
  browserModule =
    ...
    in
    {
      ...
      userDataDir = mkOption {
        ...
        default = null;
        example = literalExpression ''"''${config.xdg.configHome}/my-custom-chromium"'';
        description = ''
          Custom user data directory. If set, Home Manager will install extensions,
          dictionaries, and native messaging hosts into this directory, and the browser
          will automatically be launched with `--user-data-dir` pointing here.
  ...
  browserConfig =
    ...
    let
      ...
      actualCommandLineArgs = cfg.commandLineArgs ++
        lib.optional (cfg.userDataDir != null) "--user-data-dir=${cfg.userDataDir}";
    ...
    lib.mkIf cfg.enable {
      ...
      programs.${browser}.finalPackage =
        if actualCommandLineArgs != [ ] then
          cfg.package.override {
            commandLineArgs = lib.concatStringsSep " " actualCommandLineArgs;
          }
        else
          cfg.package;
